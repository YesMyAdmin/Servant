...
func NewBackupTask(req *backupdto.NewBackupTaskReq) error {
	err := backupRepo.NewBackupTask(backupEntity.NewReqToPO(req))
	if err != nil {
		return err
	}
	return nil
...
func EditBackupTask(req *backupdto.EditBackupTaskReq) error {
	err := backupRepo.EditBackupTask(backupEntity.EditReqToPO(req))
	if err != nil {
		return err
	}
	return nil
...
func GetBackupTask(taskId uint64) (*backupEntity.BackupTask, error) {
	...
	if (backupTaskPO == nil) {
		return nil, pkg.BackupTaskNotFoundError(taskId);
	}
	if (err != nil) {
		return nil, err
	}
...
func ListTasks(req *backupdto.ListTasksReq) (*pkg.PageableResp[backupdto.ListTasksResp], error) {
	tasks, total, err := backupRepo.ListTasks(req.PageNum, req.PageSize, req.TaskName)
	if err != nil {
	...
	for _, t := range tasks {
		resp := backupEntity.ToListTasksResp(&t)
...
func BackupTaskSwitch(taskId uint64, enabled bool) error {
	return backupRepo.SwitchBackupTask(taskId, enabled)
}

// 删除任务
func DeleteBackupTask(taskId uint64) error {
	return backupRepo.DeleteBackupTask(taskId)
}